...
const Schema = Yup.object().shape({
	...
	state: Yup.mixed().nullable(false).required('State is required'),
		...
		try {
			...
			if (res.Code === 200) {
				const obj = {
					...
					levelOfPurpose: JSON.parse(res.Data?.LevelPurpose ?? '[]').map(
						(id) => {
							return [...state.levelOfPurposeOptions].find(
								(level) => level.ID === id,
							);
						},
					),
					levelOfEducation:
						[...state.levelOfEducationOptions].find(
							(option, index) => option.ID === res.Data?.LevelOfEducation,
						) ?? null,
					state:
						[...state.stateOptions].find(
							(option, index) => option.ID === res.Data?.State,
						) ?? null,
					email: res.Data?.Email ?? '',
					timeZone:
						[...state.timeZoneOptions].find(
							(option, index) => option.ID === res.Data?.TimezoneID,
						) ?? null,
					...
					location:
						[...state.locationOptions].find(
							(option, index) => option.ID === res.Data?.Location,
						) ?? null,
				};
				// console.log(obj);
									...
									<Controller
										as={
											<Select
												...
												className={`${
													!!errors && !!errors.state ? 'error-form' : ''
												}`}
											/>
										}
										control={control}
									...
									<Controller
										as={
											<Select
												...
												isSearchable={true}
												isLoading={
													state.timeZoneOptions.length > 0 ? false : true
												}
